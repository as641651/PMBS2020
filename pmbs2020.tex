...
\begin{abstract}
  ...
  conditions.
  \p{everywhere: replace $...$ and $..$ with $\dots$} 
\end{abstract}
...
In practice, one has to consider and compare more than just two algorithms.  For instance, for the generalized least square problem $y := (X^TS^{-1}X)^{-1}X^{T}S^{-1}z$, it is possible to find more than 100 different algorithms that compute the solution and that differ not more than 1.4x in terms of FLOP count.\footnote{\ar{reference to github repo containing the julia code}}  
...
In Sec.~\ref{sec:app}, we discuss how this research fits in a bigger picture, and finally, in Sec.~\ref{sec:con}, we
...
A general approach to identify the best candidate is by ranking them according to their predicted the performance.
For linear algebra computations, the
most common performance metric to be used as performance predictor is the FLOP count, even thought it is known that the
...
reproducibility. \p{these sentences are not connected}
Many authors have noted the lack of textbook statistical behaviours in execution times, and it is not realistic to eliminate entirely performance variations~\cite{robustbenchmarking,trackingPerfVariation,statiscalperfCompare}. 
...
capture the speed up of one algorithm over the other; instead, theyt separate the fastest algorithm(s) from the rest.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		...
		\State return $(\mathcal{F} \times C)$
              \end{algorithmic}
              \p{for argmin, use the code here. j should go under}
%                \usepackage{amsmath}
...
equivalent linear algebra algorithms $\mathbf{a}_1,\mathbf{a}_2, ..., \mathbf{a}_p \in \mathcal{A}$ and assigning them
to one of the equivalence classes $r_1, r_2, ...r_w$. The number of classes (or ranks) is $w \le p$ as more than one
algorithm can obtain the same rank. For illustration, we consider an example where four algorithms
$\mathbf{a}_1,\mathbf{a}_2, \mathbf{a}_3, \mathbf{a}_4$ could be sorted and ranked as $< (\mathbf{a}_2,1),
(\mathbf{a}_4,1), (\mathbf{a}_1,2), (\mathbf{a}_3,2) >$. \p{Since we are already using $<$ and $>$ a whole lot, I think
  you should replace everywhere $< (\mathbf{a}_2,1), (\mathbf{a}_4,1), (\mathbf{a}_1,2), (\mathbf{a}_3,2) >$
  with $\left\{(\mathbf{a}_2,1), (\mathbf{a}_4,1), (\mathbf{a}_1,2), (\mathbf{a}_3,2)\right\}$.}
  All the algorithms with rank 1 are assigned to the set of fastest algorithms $\mathcal{F}$. The assignments of ranks
  need not be deterministic. So in the second step, \p{which ``second step''? Maybe you mean: ``Hence, in a second step, ...''} we bootstrap and repeat this procedure $T$ times and all the algorithms that obtained rank 1 at least once are assigned to $\mathcal{F}$. Then, we follow the steps from line 5 - 9 in Procedure \ref{alg:fa} to compute relative scores. 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		...
		\State return $<$ \p{for readability: return ``$<$''?}

                \p{or
                return $\mathbf{a_i} < \mathbf{a_j}$ } \Comment{$\mathbf{a_i}$ is better than $\mathbf{a_j}$}
...
\subset \mathbf{a_i}$ and $\mathbf{\tilde{a}_j} \subset \mathbf{a_j}$) with sample size $K < N$ \p{or $K \le N$?}  and the minimum
execution time from the respective samples are compared. This procedure is repeated $M$ times.
\p{can we quantify $M$ in any way? $M\ge 1$ is obvious. What else? $1 \le M \le N ???$, anything?} 
In theory, one could
...
\lor 1$ \p{$\lor$ is the wrong symbol. This is a logical ``or''. You instead want to list two elements, that is, $0 | 1$} and outcome B
($\sim$) becomes less and less likely \p{what does $\sim$ mean here?}. When $K=N$, \p{before we wrote that $K<N$. Maybe replace with $K \le N$ ?} outcome B becomes impossible and this invalidates the advantages of bootstrapping. When $K \to 1$, the minimum estimates of samples ($e_i, e_j$) points to an instance of execution time ($t_i, t_j$) from the distribution. Then outcome B becomes more likely even for marginal overlap of distributions (as in Fig \ref{fig:diff}), especially when $threshold$ is high or $M$ is small.
...
ranks $r_j \leftarrow j$ (where $j \in\{1,2,\dots,p\}$) according to the order in which they are given as input $<1,2,...,p>
$.
...
$\mathcal{S} : < \mathbf{s}_1, \mathbf{s}_2, ..., \mathbf{s}_p >$. In the beginning, $s_j \leftarrow j$ and $
\mathbf{a}_{s[j]} \leftarrow \mathbf{a}_j$.
\p{I got lost. What is ${a}_{s[j]}$? What do the [ ] mean?} 
Thus, Procedure \ref{alg:sort} sorts the initial sequence $< (\mathbf{a}_1,1), .., (\mathbf{a}_p,p) >$ and returns $< (\mathbf{a}_{s[1]},1), .., (\mathbf{a}_{s[p]},w) >$.
...
% and slots $s_j \leftarrow j$ , $ \mathbf{a}_{s[j]} \leftarrow \mathbf{a}_j$ where $j \in\{1,2,..p\}$; as more than one algorithm can obtain the same rank, we use different indices for positions (slots) of the algorithms ($s_j$) in the sorted sequence and ranks ($r_j$).  
 ...
 indices if the algorithm occurring later in the sequence $\mathbf{a}_{s[j+1]}$ is better than its previous algorithm
 $\mathbf{a}_{s[j]}$ \p{do not use ``previous''. Alternatives: ``preceeding''? ``the algorithm to its left''?
   ``predecessor''?}and the ranks are updated. To understand the rank update rules, consider the example in Fig
...
\begin{enumerate}
	\item 
          ...
          element in sequence. Currently, the sequence is $<\mathbf{a}_1, \mathbf{a}_2, \mathbf{a}_3, \mathbf{a}_4>$.
	...
	\item Next, algorithm $\mathbf{a}_1$ is compared with $\mathbf{a}_3$;
          \p{should we write:
            "Next, algorithm $\mathbf{a}_1$ is compared with its right neighbor (or with its successor), $\mathbf{a}_3$";}
       ...
       \item
         \p{I would start with: ``In the last comparison of the first sweep, [line xyz], Algorithm $\mathbf{a}_4$ results
           to be better ...'' -- and then remove ``With this step, a single pass.... completed.''}
         Algorithm $\mathbf{a}_4$ results to be better than $\mathbf{a}_3$, so their positions are
          ...
          With this step, a single pass of bubble sort has been completed.
          Every pass of bubble sort pushes the slowest algorithm to the end of the sequence.
          At this point, the current sequence is $<\mathbf{a}_2, \mathbf{a}_1, \mathbf{a}_4, \mathbf{a}_3>$.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		...
		\State return $(\mathcal{F} \times C)$
              ...
              \p{syntax still to be fixed $==$, square brackets. Also SPELL CHECK everything!}
\end{algorithm}
