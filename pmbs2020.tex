...
%		\State sample $K$ values $\mathbf{\tilde{a}_{j}} \subset \mathbf{a_j} \quad \forall j$ \Comment{$ j \in \{1,\dots,p\}$} 
		\State $ f \leftarrow \underset{j}{\mathrm{argmin}}(\min(\mathbf{\tilde{a}}_j) | \forall j)$ \Comment{$\mathbf{\tilde{a}}_j \in \mathbb{R}^{K}$}
...
%                of such alg. -- I feel that a short comment is needed}
		...
		\For{i = 1, $\dots$, $q$ }
        \State $c \leftarrow$ number of occurrences of $f_i$ in $L$ \Comment{$f_i \in \mathcal{F}$}
		\State append($c/T, C$)
		\EndFor
...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		\State $L, C \leftarrow [ \quad ]$ \Comment{Initialize empty lists}
		\For{i = 1, $\dots$, $T$}
		\State $\langle (\mathbf{a}_j, r_j) | \forall j \rangle  \leftarrow Sort(\mathcal{A})$ \Comment{$ j \in \{1,\dots,p\}$}
...
each algorithm. 
%In Table~\ref{tab:3}, we report the average precision and recall (explained shortly) of
